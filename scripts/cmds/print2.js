module.exports = {
	...
	onStart: async function ({ api, event, args, usersData }) {
		...
		registerFont(__dirname + '/fonts/Higher.ttf', { family: 'Higher' });

		// Set font properties
		ctx.font = 'bold 60px Higher';
		...
		const textHeight = 50; // assume a fixed height for the text
		
		// Draw the text on the image
		ctx.fillText(text, 200, canvas.height - textHeight - 4);
		ctx.strokeText(text, 200, canvas.height - textHeight - 4);
